...
public class MpSistemaConfigs implements Serializable {
	...
	public List<MpSistemaConfig> porParametros(String parametro) {
		//
		return this.manager.createQuery("from MpSistemaConfig where lower(parametro) = :parametro " +
																						" ORDER BY descricao", 
				MpSistemaConfig.class).setParameter("parametro", parametro.toLowerCase()).getResultList();
	}
	
	public List<MpSistemaConfig> listAll() {
		//
		return this.manager.createQuery("from MpSistemaConfig ORDER BY parametro", 
																MpSistemaConfig.class).getResultList();
	}
	...
	public MpSistemaConfig guardar(MpSistemaConfig mpSistemaConfig) {
		//
		try {
			mpSistemaConfig = this.manager.merge(mpSistemaConfig);
	...
	public void remover(MpSistemaConfig mpSistemaConfig) throws MpNegocioException {
		...
		try {
			...
			this.manager.remove(mpSistemaConfig);
			this.manager.flush();
			//
...
//		System.out.println("MpSistemaConfigs.porParametro ( " + parametro);
		//
		try {
			return manager.createQuery("from MpSistemaConfig where upper(parametro) = :parametro",
