...
public class MpGruposX implements Serializable {
	//
	...
	private Criteria criarCriteriaParaFiltro(MpGrupoFilter filtro) {
		...
		if (StringUtils.isNotBlank(filtro.getStatus()))
			criteria.add(Restrictions.ilike("mpStatus.descricao", filtro.getStatus(), MatchMode.ANYWHERE));
		//
		return criteria;
	}
	...
	public MpGrupo guardar(MpGrupo mpGrupo) {
		...
		try {
			...
			throw new MpNegocioException("Erro de concorrência. Esse GRUPO... já foi alterado anteriormente!");
		}
	}
	...
	public List<MpGrupo> mpGrupoList() {
		//
		return manager.createQuery("from MpGrupo ORDER By nome", MpGrupo.class).getResultList();
	}
