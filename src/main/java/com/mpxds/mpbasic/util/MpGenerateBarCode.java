...
//			doc.add(codeEANImage);
	...
	public static String atualizaPDF(File fileX, Integer posX, Integer posY) {
		//
		System.out.println("MpGenerateBarCode.atualizaPDF() - 000 ( ArqX.= " + fileX.getPath()); 
		//
		...
		try {
			...
			pdfStamper = new PdfStamper(pdfReader, new FileOutputStream(fileX.getPath().replace(fileX.getName(),
																							"out_" + fileX.getName())));

			PdfContentByte content = pdfStamper.getUnderContent(1); // 1 for the first page ...
			//
			String numCerp = capturaNumCerp(pdfReader);
			if (numCerp.indexOf("Erro") >= 0)
				return numCerp;			
...
//			content.endText();
			...
			pdfReader.close();
			//
			System.out.println("MpGenerateBarCode.atualizaPDF() - 001 ( ArqX.= " + fileX.getPath());

		} catch (DocumentException dex) {
			return "Erro - Exception ! ( ex = " + dex;
		} catch (Exception ex) {
			return "Erro - Exception ! ( ex = " + ex;
		}
		//
		return "Geração QRCode ... efetuada ! ( " + fileX.getPath().replace(fileX.getName(), "out_" + fileX.getName());
	}
	...
	private static String capturaNumCerp(PdfReader pdfReader) {
		//
		String numCerp = "";

		try {
			String pageTxtPDF = PdfTextExtractor.getTextFromPage(pdfReader, 1);

			Integer posNumCerp = pageTxtPDF.indexOf("CERP: ");
			if (posNumCerp >=0) {
				numCerp = pageTxtPDF.substring(posNumCerp + 5).trim();
			} else
				return "Erro - Captura Número CERP !";
			//
		} catch (IOException e) {
			return "Erro - Exception ! ( e = " + e;
		}
		//
		return numCerp;
	}
